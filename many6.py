...
def save_information(contact=None, names=None, pressed_btn=None, userr_id=None, yonalish=None, country=None, email=None):
    ...
    if pressed_btn:
        ...
        sql = f"""Update Log set country="{country}" where user_id={userr_id}"""
    elif names:
...
def start_button_markup():
    ...
    reply_markup.row(
        telebot.types.KeyboardButton(text="🤖IRC Classic (8-12)yosh")
    )
    buttons = [
        "🤖IRC Robotics (13-16)yosh"
    ]
...
def echo_message(message):
    ...
    if message.text == "🤖IRC Classic (8-12)yosh":
    ...
    elif message.text == "🤖IRC Robotics (13-16)yosh":
        text = "🗂Sizni qiziqtirgan bo'limni tanlang"
...
def start_button(message):
    ...
    bot.send_message(message.from_user.id, f"🥳{home} sizni Ro'yxatdan o'tganingiz bilan tabriklaymiz. \n"
                                           f"Agar ro'yxatdan notug'ri o'tgan bo'lsangiz boshqatdan ro'yxatdan o'tishingizni iltimos qilamiz.",
                     reply_markup=start_button_markup())
